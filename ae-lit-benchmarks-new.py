...
logfile = "lit-log-file-benchmarks-bag.txt"

# OUTPUT FILE
prefname = "lit-benchmarks-bag"

# TIMEOUT (in seconds)
cmdtimeout = 1500 #

fsmfiles = []
for filename in os.listdir("tests/benchmarks"):
    if filename.endswith(".txt") and not filename.startswith("elevator-extra"):
        fsmfiles.append("tests/benchmarks/" + filename)
        ...
        with open(name+logfile, "ab") as log_file:
            for ex in filelist:
                ...
                for it in range(1,maxiterations):
                    ...
                    #
                    #
                    kmccmd = subprocess.run(["cabal", "run", "KMC", "--",ex,str(it),filetype,], capture_output=True, text=True, check=True,) #stdout=subprocess.PIPE
                    try:
...
outputfile = "benchmarks-fromlit-bag.csv"
